...
class TwoColumnCoverLetterTemplate(TemplateInterface):
    """
    Helper class for generating a Two-Column LaTeX cover letter from JSON data.
    Features a sidebar with contact info, main content with letter content.
    Handles special characters: &, %, $, #
    """

    def __init__(self, data: dict[str, Any], config: dict[str, Any] = None) -> None:
        """
        Initialize the TwoColumnCoverLetterTemplate class.
    ...
    def generate_sidebar_contact(self) -> str:
        ...
        if info.get("location"):
            contact_lines.append(r"\contactitem{Location}{" + str(info["location"]) + r"}")
    ...
    def generate_name(self):
        ...
        return self.data["personalInfo"]["name"]
    ...
    def export_to_docx(self, output_path: str = "output.docx") -> str:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                raise TemplateRenderingException(
                    template_name="two_column_cover_letter",
