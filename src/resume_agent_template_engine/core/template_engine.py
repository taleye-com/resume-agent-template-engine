...
class TemplateConfig:
    ...
    def get_template_config(self, document_type: str, template_name: str) -> Dict[str, Any]:
        """Get configuration for a specific template"""
        return self.config.get("templates", {}).get(document_type, {}).get(template_name, {})
...
class TemplateRegistry:
    ...
    def get_available_templates(self, document_type: Optional[str] = None) -> Union[Dict[str, List[str]], List[str]]:
    ...
    def load_template_class(self, document_type: str, template_name: str) -> type:
        ...
        if template_name not in self._available_templates[document_type]:
            raise ValueError(f"Template '{template_name}' not found for {document_type}")
        
        # Load the template class
        helper_path = self.templates_base_path / document_type / template_name / "helper.py"
        
        spec = importlib.util.spec_from_file_location(f"{document_type}_{template_name}", helper_path)
    ...
    def _find_template_class(self, module, document_type: str, template_name: str) -> type:
        ...
        template_classes = [name for name in dir(module) if name.endswith('Template') and not name.startswith('_')]
        if template_classes:
...
class TemplateEngine:
    ...
    def __init__(self, config_path: Optional[str] = None, templates_path: Optional[str] = None):
        """
        ...
        if templates_path:
            ...
            self.templates_path = self.config.config.get("templates", {}).get("base_path", "templates")
        
        self.registry = TemplateRegistry(self.templates_path)
        logger.info(f"TemplateEngine initialized with {len(self.get_available_templates())} template types")
    ...
    def create_template(self, document_type: str, template_name: str, data: Dict[str, Any]) -> TemplateInterface:
        """
    ...
    def render_document(self, 
                       ...
                       template_name: str, 
                       data: Dict[str, Any],
                       output_format: OutputFormat = OutputFormat.LATEX) -> str:
        """
    ...
    def get_template_info(self, document_type: str, template_name: str) -> Dict[str, Any]:
        """
