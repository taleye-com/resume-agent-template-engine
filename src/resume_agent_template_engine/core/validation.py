    ...
    def sanitize_text(cls, text: str, allow_basic_formatting: bool = True) -> str:
        ...
        for dangerous_pattern in cls.DANGEROUS_COMMANDS:
            if re.search(dangerous_pattern, text, re.IGNORECASE):
                logger.warning(f"Dangerous LaTeX command detected and removed: {dangerous_pattern}")
...
class DataNormalizer:
    ...
    @staticmethod
    def normalize_date(date_input: Union[str, dict, None]) -> Tuple[str, List[ValidationError]]:
        """
        ...
        if isinstance(date_input, dict):
            ...
            if not year:
                errors.append(ValidationError.create(
        ...
        if isinstance(date_input, str):
            ...
            date_patterns = [
                ...
                (r'^(\d{4})-(\d{1,2})$', lambda m: f"{int(m.group(1)):04d}-{int(m.group(2)):02d}"),
                (r'^(\d{4})$', lambda m: f"{int(m.group(1)):04d}"),
                (r'^(\d{1,2})/(\d{1,2})/(\d{4})$', lambda m: f"{int(m.group(3)):04d}-{int(m.group(1)):02d}-{int(m.group(2)):02d}"),
                (r'^(\d{1,2})/(\d{4})$', lambda m: f"{int(m.group(2)):04d}-{int(m.group(1)):02d}"),
                (r'^(\w+)\s+(\d{4})$', lambda m: DataNormalizer._parse_month_year(m.group(1), m.group(2))),
                (r'^(\w+)\s+(\d{1,2}),?\s+(\d{4})$', lambda m: DataNormalizer._parse_month_day_year(m.group(1), m.group(2), m.group(3))),
            ]
    ...
    def normalize_phone(phone: str) -> Tuple[str, List[ValidationError]]:
        ...
        if phone_clean.startswith('+'):
            ...
            normalized = phone_clean[:2] + ' ' + phone_clean[2:]  # +1 1234567890 -> +1 1234567890
            if len(phone_clean) == 12 and phone_clean.startswith('+1'):  # US format
                normalized = f"+1 ({phone_clean[2:5]}) {phone_clean[5:8]}-{phone_clean[8:]}"
        else:
...
class ResumeValidator:
    ...
    def _process_sections(self, data: Dict[str, Any], errors: List[ValidationError], warnings: List[ValidationError]) -> Dict[str, Any]:
        """Process and normalize all sections"""
        normalized = {}

        # Process personalInfo
        if "personalInfo" in data:
            normalized["personalInfo"] = self._process_personal_info(data["personalInfo"], errors, warnings)

        # Process experience section
        if "experience" in data:
            normalized["experience"] = self._process_experience(data["experience"], errors, warnings)

        # Process education section
        if "education" in data:
            normalized["education"] = self._process_education(data["education"], errors, warnings)
    ...
    def _process_personal_info(self, personal_info: Dict[str, Any], errors: List[ValidationError], warnings: List[ValidationError]) -> Dict[str, Any]:
        ...
        for field, value in personal_info.items():
            if field == "email":
                ...
                self._add_field_errors(field_errors, f"personalInfo.{field}", errors, warnings)
    ...
    def _process_experience(self, experience: List[Dict[str, Any]], errors: List[ValidationError], warnings: List[ValidationError]) -> List[Dict[str, Any]]:
        ...
        for i, exp in enumerate(experience):
            ...
            for field, value in exp.items():
                if field in ["startDate", "endDate"]:
                    normalized_value, field_errors = self.normalizer.normalize_date(value)
                    self._add_field_errors(field_errors, f"experience[{i}].{field}", errors, warnings)
    ...
    def _process_education(self, education: List[Dict[str, Any]], errors: List[ValidationError], warnings: List[ValidationError]) -> List[Dict[str, Any]]:
        ...
        for i, edu in enumerate(education):
            ...
            for field, value in edu.items():
                if field in ["startDate", "endDate", "graduationDate"]:
                    normalized_value, field_errors = self.normalizer.normalize_date(value)
                    self._add_field_errors(field_errors, f"education[{i}].{field}", errors, warnings)
    ...
    def _process_generic_section(self, section_data: Any, section_path: str, errors: List[ValidationError], warnings: List[ValidationError]) -> Any:
    ...
    def _add_field_errors(self, field_errors: List[ValidationError], field_path: str, errors: List[ValidationError], warnings: List[ValidationError]) -> None:
    ...
    def _sanitize_all_text(self, data: Any, path: str = "") -> Any:
        ...
        if isinstance(data, dict):
            return {key: self._sanitize_all_text(value, f"{path}.{key}" if path else key) for key, value in data.items()}
        elif isinstance(data, list):
            return [self._sanitize_all_text(item, f"{path}[{i}]") for i, item in enumerate(data)]
...
def validate_resume_data(data: Dict[str, Any], validation_level: ValidationLevel = ValidationLevel.LENIENT) -> ValidationResult:
    """
