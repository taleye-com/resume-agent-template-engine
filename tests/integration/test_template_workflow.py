...
"""
    ...
    def test_document_generation_workflow(
        self, template_engine_with_mock_templates, tmp_path
    ):
        ...
        cover_letter_data = {
            "personalInfo": {"name": "John Doe", "email": "john@example.com"},
            "recipient": {"name": "Jane Smith", "company": "Target Corp"},
        }

        cover_letter_content = engine.render_document(
            "cover_letter", "formal", cover_letter_data
        )
        assert isinstance(cover_letter_content, str)
        assert "documentclass" in cover_letter_content

    def test_template_info_retrieval_workflow(
        self, template_engine_with_mock_templates
    ):
    ...
    def test_error_handling_workflow(self, template_engine_with_mock_templates):
        ...
        with pytest.raises(ValueError, match="Recipient info required"):
            engine.create_template(
                "cover_letter", "formal", incomplete_cover_letter_data
            )
...
class TestAPIIntegration:
    ...
    @patch("os.remove")
    def test_api_template_engine_integration(
        self,
        mock_remove,
        mock_exists,
        mock_tempfile,
...
"""
    ...
    def test_complete_workflow_from_config_to_pdf(self, complete_setup):
        ...
        engine = TemplateEngine(
            config_path=setup["config_file"], templates_path=setup["templates_dir"]
        )
