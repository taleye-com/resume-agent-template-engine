...
from resume_agent_template_engine.core.template_engine import TemplateEngine, OutputFormat
...
class TestCLIFunctions:
    ...
    def test_generate_document_pdf_success(self, mock_load_data):
        ...
        with patch('builtins.print') as mock_print:
            generate_document(mock_engine, "resume", "classic", "data.json", "output.pdf", "pdf")
    ...
    def test_generate_document_latex_success(self, mock_file, mock_load_data):
        ...
        with patch('builtins.print') as mock_print:
            generate_document(mock_engine, "resume", "classic", "data.json", "output.tex", "latex")
        
        mock_engine.validate_template.assert_called_once_with("resume", "classic")
        mock_engine.render_document.assert_called_once_with("resume", "classic", mock_load_data.return_value, OutputFormat.LATEX)
    ...
    def test_generate_document_invalid_template(self, mock_load_data):
        ...
        with pytest.raises(SystemExit):
            generate_document(mock_engine, "resume", "nonexistent", "data.json", "output.pdf", "pdf")
    ...
    def test_generate_document_unsupported_format(self, mock_load_data):
        ...
        with pytest.raises(SystemExit):
            generate_document(mock_engine, "resume", "classic", "data.json", "output.html", "html")
...
class TestCLIMain:
    ...
    @patch('sys.argv', ['cli.py', 'generate', 'resume', 'classic', 'data.json', 'output.pdf'])
    @patch('resume_agent_template_engine.cli.TemplateEngine')
    ...
    def test_main_generate_command(self, mock_generate, mock_engine_class):
        ...
        mock_engine_class.assert_called_once()
        mock_generate.assert_called_once_with(mock_engine, 'resume', 'classic', 'data.json', 'output.pdf', 'pdf')

    @patch('sys.argv', ['cli.py', 'generate', 'resume', 'classic', 'data.json', 'output.tex', '--format', 'latex'])
    @patch('resume_agent_template_engine.cli.TemplateEngine')
    @patch('resume_agent_template_engine.cli.generate_document')
    def test_main_generate_command_latex(self, mock_generate, mock_engine_class):
        ...
        mock_engine_class.assert_called_once()
        mock_generate.assert_called_once_with(mock_engine, 'resume', 'classic', 'data.json', 'output.tex', 'latex')

    @patch('sys.argv', ['cli.py', '--verbose', 'list'])
    @patch('resume_agent_template_engine.cli.TemplateEngine')
    @patch('resume_agent_template_engine.cli.list_templates')
    @patch('resume_agent_template_engine.cli.setup_logging')
    def test_main_verbose_flag(self, mock_setup_logging, mock_list_templates, mock_engine_class):
    ...
    @patch('sys.argv', ['cli.py', '--config', 'config.yaml', '--templates-path', '/custom/templates', 'list'])
    @patch('resume_agent_template_engine.cli.TemplateEngine')
    @patch('resume_agent_template_engine.cli.list_templates')
    def test_main_with_config_and_templates_path(self, mock_list_templates, mock_engine_class):
