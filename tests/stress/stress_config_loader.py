...
class StressTestConfig:
    ...
    def __init__(self, config_path: Optional[str] = None, stress_level: Optional[str] = None):
        """
    ...
    def _get_default_stress_level(self) -> str:
        ...
        return self.config.get('global_settings', {}).get('default_stress_level', 'medium')
    ...
    def _apply_environment_overrides(self):
        ...
        overrides = self.config.get('environment_overrides', {}).get(self.environment, {})
        ...
        if overrides:
            ...
            if 'concurrent_requests_multiplier' in overrides:
                ...
                for key, value in stress_config['concurrent_requests'].items():
                    if isinstance(value, (int, float)):
                        stress_config['concurrent_requests'][key] = max(1, int(value * multiplier))
                    elif isinstance(value, list):
                        stress_config['concurrent_requests'][key] = [max(1, int(v * multiplier)) for v in value]
            ...
            if 'duration_multiplier' in overrides:
                ...
                for key, value in stress_config['test_duration'].items():
                    stress_config['test_duration'][key] = max(1, int(value * multiplier))
            
            # Apply timeout override
            if 'timeout_seconds' in overrides:
                self.config['global_settings']['timeout_seconds'] = overrides['timeout_seconds']
    ...
    def get_concurrent_requests(self, test_name: str, level: Optional[str] = None) -> int:
    ...
    def get_performance_requirement(self, requirement: str, level: Optional[str] = None) -> float:
    ...
    def get_payload_size_config(self, level: Optional[str] = None) -> Dict[str, Any]:
        ...
        config = self.get_stress_level_config(level)
        return config.get('payload_size', {'large_payload_entries': 100, 'large_payload_size_mb': 5.0})
    
    def get_scaling_test_levels(self, level: Optional[str] = None) -> List[int]:
        """Get scaling test levels."""
        config = self.get_stress_level_config(level)
        return config.get('concurrent_requests', {}).get('scaling_test_levels', [5, 10, 15, 20, 25])
    ...
    def should_skip_test(self, test_name: str, category: str = 'staging') -> bool:
        ...
        if category_config.get('skip_slow_tests', False):
            slow_tests = ['sustained_load', 'recovery_after_overload', 'scaling_behavior']
    ...
    def is_detailed_logging_enabled(self) -> bool:
        ...
        return self.config.get('global_settings', {}).get('enable_detailed_logging', True)
    ...
    def print_configuration_summary(self):
        ...
        if config:
            ...
            print(f"High Volume Test: {self.get_concurrent_requests('high_volume_test')} concurrent requests")
            print(f"Success Rate Threshold: {self.get_performance_requirement('success_rate_threshold'):.1%}")
            print(f"Avg Response Time Threshold: {self.get_performance_requirement('avg_response_time_threshold'):.1f}s")
            print(f"Throughput Threshold: {self.get_performance_requirement('throughput_threshold'):.1f} req/s")
        
        print("=" * 45)


def get_stress_config(stress_level: Optional[str] = None, config_path: Optional[str] = None) -> StressTestConfig:
    """
...
def export_test_metrics(metrics: Dict[str, Any], config: StressTestConfig, test_name: str):
